object FLotesItems: TFLotesItems
  Left = 0
  Top = 0
  BorderIcons = []
  BorderStyle = bsDialog
  Caption = 'Lotes do Produto'
  ClientHeight = 352
  ClientWidth = 570
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poMainFormCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object PanelLotesItems: TPanel
    Left = 0
    Top = 0
    Width = 570
    Height = 90
    Align = alTop
    TabOrder = 0
    object lbNMProduto: TLabel
      Left = 9
      Top = 20
      Width = 334
      Height = 22
      Caption = 'Nome do Produto Nome do Produto '
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -19
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label4: TLabel
      Left = 8
      Top = 7
      Width = 102
      Height = 13
      Caption = 'Descri'#231#227'o do Produto'
    end
    object Label1: TLabel
      Left = 9
      Top = 45
      Width = 56
      Height = 13
      Caption = 'Quantidade'
    end
    object lbQtdProduto: TLabel
      Left = 9
      Top = 60
      Width = 294
      Height = 22
      Caption = 'Qtd do Produto Qtd do Produto '
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -19
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 90
    Width = 570
    Height = 262
    Align = alClient
    Color = clMoneyGreen
    TabOrder = 1
    object GroupBox4: TGroupBox
      Left = 1
      Top = 1
      Width = 568
      Height = 224
      Align = alClient
      Caption = 'Dados dos Lotes'
      Color = clBtnFace
      ParentColor = False
      TabOrder = 0
      object wwDBGrid1: TwwDBGrid
        Left = 2
        Top = 15
        Width = 564
        Height = 207
        Selected.Strings = (
          'NLOTE'#9'20'#9'N'#186'. Lote'#9'F'
          'QLOTE'#9'19'#9'Qtd. Lote'#9'F'
          'DFAB'#9'10'#9'Data Fabrica'#231#227'o'#9'F'
          'DVAL'#9'10'#9'Data Vencimento'#9'F'
          'VPMC'#9'19'#9'R$ M'#225'x. Consumidor'#9'F')
        IniAttributes.Enabled = True
        IniAttributes.Delimiter = ';;'
        IniAttributes.CheckNewFields = True
        TitleColor = clBtnFace
        FixedCols = 0
        ShowHorzScrollBar = True
        Align = alClient
        DataSource = DataSource1
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit, dgWordWrap]
        TabOrder = 0
        TitleAlignment = taCenter
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        TitleLines = 1
        TitleButtons = False
        UseTFields = False
        PaintOptions.AlternatingRowColor = 15000804
      end
    end
    object Panel3: TPanel
      Left = 1
      Top = 225
      Width = 568
      Height = 36
      Align = alBottom
      BevelOuter = bvLowered
      TabOrder = 1
      object Bt_IncluirItem: TBitBtn
        Left = 215
        Top = 6
        Width = 110
        Height = 25
        Caption = 'Incluir Lote - F2'
        DoubleBuffered = True
        Glyph.Data = {
          
          0E0E0E0E0E0E0B08171B0C1B1B1B1B1B1B1B1B1B1B1B1B1B1B0C}
        ParentDoubleBuffered = False
        TabOrder = 0
        TabStop = False
      end
      object Bt_ExcluirItem: TBitBtn
        Left = 331
        Top = 6
        Width = 110
        Height = 25
        Caption = 'Excluir Lote - F4'
        DoubleBuffered = True
        Glyph.Data = {
          
          0E0E0E0E0E0E0B08171B0C1B1B1B1B1B1B1B1B1B1B1B1B1B1B0C}
        ParentDoubleBuffered = False
        TabOrder = 1
        TabStop = False
        OnClick = Bt_ExcluirItemClick
      end
      object Bt_Ok: TBitBtn
        Left = 454
        Top = 4
        Width = 99
        Height = 28
        Caption = 'Ok - F10'
        DoubleBuffered = True
        Glyph.Data = {
          
          1B1B1B1B1B1B1B1B1B1B1B1B1B1B1B1B1B1B1B1B1B1B1B1B1B1B}
        ParentDoubleBuffered = False
        TabOrder = 2
        OnClick = Bt_OkClick
      end
    end
  end
  object DataSource1: TDataSource
    DataSet = DtM.IBNFLotesItems
    Left = 40
    Top = 264
  end
end
