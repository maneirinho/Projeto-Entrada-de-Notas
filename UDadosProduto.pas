    ...
    begin
      ...
      try
        ...
        on E: exception do begin
          ...
          MensagemDeErro('Não foi possível baixar este item de pedido de compra');
        end;
      end;
    end;
  end;
...
begin
  ...
  edCustoAtual.Hint := 'Último Custo R$ ' + FormatCurr(',0.00',UltimoCustoProd) + PL + 'Último Custo Médio R$ ' + FormatCurr(',0.00',UltimoCustoMedioProd);
end;
...
begin
  ...
  begin
    result := true;
    MensagemDeAtencao('É necessário informar o Preço de Venda');
  ...
  begin
    ...
    begin
      result := true;
      MensagemDeAtencao('A quantidade do ITEM não confere com a quantidade da GRADE');
      btGrade.Click;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        result := not MensagemDeConfirmacao('A quantidade do ITEM não confere com a quantidade de NÚMEROS DE SÉRIE'+PL+PL+'Continua assim mesmo?', 'ATENÇÃO !!!', mbNo) = mrYes;
...
begin
  ...
  begin
    ...
    //essa linha tá salvando no diretório o arquivo SQLTeste.txt
    ...
    // Cria o vínculo do Código do Produto do Fornecedor
...
//      'U': rbCustoAtual.Checked := true;
  ...
  cbUtilizaCustoMedio.Hint := 'Custo Médio R$ ' + FormatCurr(',0.00',CustoMedioProd) + PL + 'Custo Atual R$ ' + FormatCurr(',0.00',CustoAtualProd);
end;
...
begin
  Application.CreateForm(TfTabela, fTabela);
  fTabela.Caption := 'Últimas Compras - '+ lbCodigo.Caption+ ' - '+edNMProduto.Text;
  fTabela.ChaveEntrada := CodigoDoProduto;
...
begin
  Application.CreateForm(TfTabela, fTabela);
  fTabela.Caption := 'Últimas Vendas - '+ lbCodigo.Caption+ ' - '+edNMProduto.Text;
  fTabela.ChaveEntrada := CodigoDoProduto;
...
begin
  ...
  begin
    MensagemDeAtencao('Preço Máximo ao Consumidor (R$ '+FormatCurr(',0.00',FLotesItems.ValorMax)+') Menor do que o Preço de Venda! Verifique');
    edPrecoVenda.SetFocus;
