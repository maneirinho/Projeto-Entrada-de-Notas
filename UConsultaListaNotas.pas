...
uses
  ...
  ACBrNFe, ACBrUtil, pcnConversao, UConstVar, JvComponentBase, JvBalloonHint;
const
...
type
  ...
  TfEntradasDeNotas = class(TForm)
    ...
    procedure btnTipoXMLClick(Sender: TObject);
  private
  ...
  public
    ...
    ArquivoXML      : string;
    TipoNota  : TTipoNota;
    Margem_Adicional_Item: Currency;
  end;
...
uses UConsultaNotas, UNovaEntradaNotas, UItemsEntradaNotas,
...
var
  i : integer;
  DataTemp, EstaPasta: String;
  Agora : TDateTime;
begin
  RegChaveForms := 'SOFTWARE\FOCUS\FORMS';
  RegProduto.Foto := TBitmap.Create;

  EstaPasta  := PathComBarra(DiretorioDoExecutavel);
  INIGrids   := EstaPasta+'Grids.INI';
...
procedure TfEntradasDeNotas.btnTipoXMLClick(Sender: TObject);
begin
  if not TeclaTaPressionada(VK_CONTROL) then
    Exit;
  if btnTipoXML.Caption = 'C' then
  begin
    TipoNota := tpnMeia;
    btnTipoXML.Caption := 'M';
  end
  else
  begin
    TipoNota := tpnCheia;
    btnTipoXML.Caption := 'C';
  end;
end;
...
begin
  ...
  FechaAviso;
  Screen.Cursor := crDefault;
  if btnTipoXML.Caption = 'C' then TipoNota := tpnCheia
                              else TipoNota := tpnMeia;
end;
              ...
              if fNovaEntradaNota.BotaoPressionado <> bpOk then Exit
                ...
                begin
                  try
                    ...
                    IBTabela.SQL.Add(' CHAVE_NFE, VALOR_II, VALOR_PIS, VALOR_COFINS, ACRES_DESC_ESPECIAL)');
                    IBTabela.SQL.Add('Values');
                    IBTabela.SQL.Add('(:CD_NOTA, :TIPO_NOTA, :CD_FORNECEDOR, :CNPJ, :NM_FORNECEDOR, :NUM_DOCUMENTO, ');
                    IBTabela.SQL.Add(':CFOP, :DT_EMISSAO, :DT_ENTRADA, :OBS, :ICMS_BASE, :ICMS_VALOR, ');
                    IBTabela.SQL.Add(':ICMS_BASESUBST, :ICMS_VALORSUBST, :VR_TOTAL_PROD, :VALOR_FRETE, ');
