...
uses
  ...
  JvEdit, JvLabel, MessageFunctions;
...
begin
  Panel2.Caption := IntToStr(fDB.GetQuantidadeDeRegistros(DtM.IBDatabaseServer, NomeDaTabela, Filtro));
end;
...
begin
  if Modo = mdNavegacao then Close
    ...
    case MensagemDeConfirmacao('Deseja salvar este registro antes de sair?', 'Confirmação', mbYes) of
       mrYes    : begin
                    Bt_Gravar.Click;
...
begin
  result := '';
  AbreAviso('Verificando Dependências...', 0, 0, 0, 0);
  try
    with CM do
    begin
...
begin
  ...
  try
    if MensagemDeConfirmacao('Confirma exclusão do(a) '+ CadObjeto +': '+EsteRegistro+'?') <>  mrYes then
      Exit;

    Dependencias := TemDependencias;
    if not Vazio(Dependencias) then
    begin
      if MensagemDeConfirmacao('Este '+CadObjeto+' faz referências a outros registros conforme descrito abaixo.'+PL+'Deseja realmente excluí-lo?'+PL+PL+Dependencias, 'A T E N Ç Ã O') <> mrYes then Exit;
      if MensagemDeConfirmacao('Esta operação é irreversível.'+PL+'Tem certeza que deseja eliminar este '+CadObjeto+'?', 'A T E N Ç Ã O') <> mrYes then Exit;
      if Uppercase(InputBox('Confirmação', 'Para continuar, digite "SIM"...', '')) <> 'SIM' then Exit;
    end;

    AbreAviso('Apagando Registro...');
    with CM do
    begin
      ...
      try
        ...
        on E: exception do begin
          ...
          MensagemDeErro('O Registro está sendo alterado por outro usuário. ');
        end;
      end;
    end;
  finally
...
begin
  ...
  try
    ...
    ExecutaBuscaTexto;
    if not TaAbrindoForm then FechaAviso;
    AlteraModo(mdNavegacao);
    Screen.Cursor := crDefault;
  end;
end;
