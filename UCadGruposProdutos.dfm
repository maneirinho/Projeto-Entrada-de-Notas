object fCadGruposProdutos: TfCadGruposProdutos
  Left = 227
  Top = 113
  BorderIcons = []
  BorderStyle = bsDialog
  Caption = 'Grupos de Produtos'
  ClientHeight = 439
  ClientWidth = 616
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poMainFormCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 400
    Width = 616
    Height = 39
    Align = alBottom
    TabOrder = 4
    object Bt_Incluir: TBitBtn
      Left = 8
      Top = 8
      Width = 97
      Height = 25
      Caption = 'Incluir - F2'
      DoubleBuffered = True
      Glyph.Data = {
        
        0E0E0E0E0E0E0B08171B0C1B1B1B1B1B1B1B1B1B1B1B1B1B1B0C}
      ParentDoubleBuffered = False
      TabOrder = 0
      TabStop = False
      OnClick = Bt_IncluirClick
    end
    object Bt_Excluir: TBitBtn
      Left = 108
      Top = 8
      Width = 97
      Height = 25
      Caption = 'Excluir - F4'
      DoubleBuffered = True
      Glyph.Data = {
        
        0E0E0E0E0E0E0B08171B0C1B1B1B1B1B1B1B1B1B1B1B1B1B1B0C}
      ParentDoubleBuffered = False
      TabOrder = 1
      TabStop = False
      OnClick = Bt_ExcluirClick
    end
    object Bt_Alterar: TBitBtn
      Left = 208
      Top = 8
      Width = 97
      Height = 25
      Caption = 'Alterar - F5'
      DoubleBuffered = True
      Glyph.Data = {
        
        0E0E0E0E0E0E0B08171B0C1B1B1B1B1B1B1B1B1B1B1B1B1B1B0C}
      ParentDoubleBuffered = False
      TabOrder = 2
      TabStop = False
      OnClick = Bt_AlterarClick
    end
    object Bt_Sair: TBitBtn
      Left = 508
      Top = 8
      Width = 97
      Height = 25
      Caption = 'Sair'
      DoubleBuffered = True
      Glyph.Data = {
        
        221F292929292929292929292929292929292929292929292929}
      ParentDoubleBuffered = False
      TabOrder = 4
    end
    object Bt_Ok: TBitBtn
      Left = 508
      Top = 8
      Width = 97
      Height = 25
      Caption = 'Ok'
      DoubleBuffered = True
      Glyph.Data = {
        
        FFFAFF000000FFFFFFFFFFFFFFFFFF000000}
      ParentDoubleBuffered = False
      TabOrder = 3
      TabStop = False
      OnClick = Bt_OkClick
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 373
    Width = 616
    Height = 27
    Align = alBottom
    TabOrder = 3
    object LabelCaminho: TLabel
      Left = 16
      Top = 6
      Width = 111
      Height = 19
      Caption = 'LabelCaminho'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object PanelData: TPanel
    Left = 0
    Top = 351
    Width = 616
    Height = 22
    Align = alBottom
    TabOrder = 2
    object Label34: TLabel
      Left = 213
      Top = 4
      Width = 86
      Height = 13
      Caption = 'Data de Cadastro:'
    end
    object Label41: TLabel
      Left = 419
      Top = 4
      Width = 80
      Height = 13
      Caption = #218'ltima Altera'#231#227'o:'
    end
    object DBEdit24: TDBEdit
      Left = 302
      Top = 3
      Width = 107
      Height = 16
      TabStop = False
      AutoSize = False
      Color = clBtnFace
      DataField = 'DT_CADASTRO'
      DataSource = DataSource1
      Font.Charset = ANSI_CHARSET
      Font.Color = clPurple
      Font.Height = -11
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      ReadOnly = True
      TabOrder = 0
    end
    object DBEdit32: TDBEdit
      Left = 502
      Top = 3
      Width = 107
      Height = 16
      TabStop = False
      AutoSize = False
      Color = clBtnFace
      DataField = 'ULTIMAALTERACAO'
      DataSource = DataSource1
      Font.Charset = ANSI_CHARSET
      Font.Color = clPurple
      Font.Height = -11
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      ReadOnly = True
      TabOrder = 1
    end
  end
  object dxDBTreeView: TdxDBTreeView
    Left = 0
    Top = 33
    Width = 616
    Height = 318
    ShowNodeHint = True
    RowSelect = True
    DataSource = DataSource1
    DisplayField = 'DESCRICAO'
    KeyField = 'CD_GRUPO'
    ListField = 'DESCRICAO'
    ParentField = 'CD_PAI'
    RootValue = '0'
    SeparatedSt = ' - '
    StateIndexField = 'CD_GRUPO'
    RaiseOnError = True
    Indent = 25
    Align = alClient
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    Color = 16776176
    ParentColor = False
    Options = [trCanDBNavigate, trSmartRecordCopy, trCheckHasChildren]
    SelectedIndex = -1
    TabOrder = 1
    OnDblClick = dxDBTreeViewDblClick
    ParentFont = False
  end
  object Panel4: TPanel
    Left = 0
    Top = 0
    Width = 616
    Height = 33
    Align = alTop
    TabOrder = 0
    object chTodos: TCheckBox
      Left = 16
      Top = 8
      Width = 161
      Height = 17
      Caption = '&Todos os Grupos'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      OnClick = chTodosClick
    end
  end
  object IBGrupos: TIBOQuery
    Params = <>
    ColumnAttributes.Strings = (
      'GRUPOS_PRODUTOS.CD_GRUPO=REQUIRED')
    IB_Transaction = IBOTransaction1
    KeyLinks.Strings = (
      'GRUPOS_PRODUTOS.CD_GRUPO')
    RecordCountAccurate = True
    SQL.Strings = (
      'Select * from GRUPOS_PRODUTOS'
      'order by DESCRICAO')
    FieldOptions = []
    Left = 272
    Top = 112
    object IBGruposCD_GRUPO: TIntegerField
      FieldName = 'CD_GRUPO'
      Origin = 'GRUPOS_PRODUTOS.CD_GRUPO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object IBGruposCD_PAI: TIntegerField
      FieldName = 'CD_PAI'
      Origin = 'GRUPOS_PRODUTOS.CD_PAI'
    end
    object IBGruposDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Origin = 'GRUPOS_PRODUTOS.DESCRICAO'
      Size = 80
    end
    object IBGruposDT_CADASTRO: TDateTimeField
      FieldName = 'DT_CADASTRO'
      Origin = 'GRUPOS_PRODUTOS.DT_CADASTRO'
      DisplayFormat = 'dd/mm/yyyy  hh:mm'
    end
    object IBGruposULTIMAALTERACAO: TDateTimeField
      FieldName = 'ULTIMAALTERACAO'
      Origin = 'GRUPOS_PRODUTOS.ULTIMAALTERACAO'
      DisplayFormat = 'dd/mm/yyyy  hh:mm'
    end
  end
  object DataSource1: TDataSource
    AutoEdit = False
    DataSet = IBGrupos
    OnDataChange = DataSource1DataChange
    Left = 232
    Top = 112
  end
  object IBOTransaction1: TIBOTransaction
    Isolation = tiCommitted
    Left = 312
    Top = 112
  end
end
