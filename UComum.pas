...
begin
  ...
  if not result then
  begin
    MensagemDeAtencao('A T E N Ç Ã O !' + PL+PL+
                      'É necessário a instalação do Firebird versão '+VersaoExigida+'.'+PL+PL+'Baixe o Firebird em '+PL+'http://www.firebirdsql.org');
  end;
end;
...
begin
  try
    ...
    if (not vazio(btTexto)) and (not vazio(Lista.Strings[0])) then //Se não estiver vazio ...
    begin
      if btComposta then  //Se for busca composta, cria uma condição para cada palavra da lista
      begin
        ...
        begin
          ...
          begin
            if (fDB.GetTipoDoCampo(DtM.IBDatabaseServer,btTabela, btCampos[c]) in TiposNumericos) and (not ENumero(Lista.Strings[i])) then
              NaoFazNada  // >>> Se o Campo é do Tipo Numérico, mas o texto não é, então não faz nada...
            else begin
      ...
      else begin        // Senão põe uma condição só...
        Condicao := '';
        _OR_ := '';
        for c := 0 to length(btCampos)-1 do
        begin
...
begin
  try
    ...
    begin
      MensagemDeErro('Não há conexão com o Banco de Dados!');
      Exit;
    end;
    ...
    try
      ...
      MensagemDeErro('Falha ao carregar Configurações de Cadastro');
      Exit;
    end;
  finally
    fDB.FechaTT(EstaTabela);
...
begin
  try
    ...
    begin
      ...
      if result = 0 then
      begin
//       MensagemDeErro('Não foi possível obter a Data e Hora do Servidor');
        result := now;
      end;
    end;
  end;
...
begin
  ...
  MensagemDeErro(Mensagem + PL+PL +
             'Entre em contato com o Suporte técnico:'+PL+
             '(24) 2224-1632'+PL+
             '(24) 9252-1250'+PL+
             '(24) 9226-0072'+PL+
  ...
  try
    ...
    try
      ...
      on E: exception do begin
        UltimoErro := E.Message;
        LogDeErros(E.Message);
        MensagemDeErro('Campo '+vdCampo+' não disponível em '+vdTabela+'.');
...
begin
  try
    ...
    if (MessageBox(0, 'Deseja conciliar informações de Clientes?', 'Conciliar Clientes', MB_ICONQUESTION or MB_YESNO or MB_APPLMODAL or MB_DEFBUTTON2) = idYes) then
    begin
      EstaTabela.Active := False;
      EstaTabela.SQL.Clear;
      ...
      if not EstaTabela.Prepared then EstaTabela.Prepare;
      EstaTabela.Active := true;
      AbreAviso('Conciliando informações de clientes...', 0, 0, EstaTabela.RecordCount, 0);
      ...
      FechaAviso;
      MensagemDeInformacao('Conciliação concluída...');
    end;
  finally
    fDB.FechaTT(EstaTabela);
    fDB.FreeQueryTransacao(EstaTabela, EstaTransacao);
...
var
  Cont : Integer;
begin
  if (MessageBox(0, 'Deseja conciliar informações e estoque de Produtos?', 'Conciliar Produtos', MB_ICONQUESTION or MB_YESNO or MB_APPLMODAL or MB_DEFBUTTON2) = idYes) then
  begin
    ...
    AbreAviso('Conciliando informações de Produtos...', 0, 0, IBTabela.RecordCount, 0);
...
begin
  try
    ...
    if not (MessageBox(0, PChar('Esta operação irá calcular a média de venda de cada produto ref. a '+IntToStr(Periodo)+' dias.'+#13+#10+'Deseja continuar?'), '', MB_ICONQUESTION or MB_YESNO or MB_APPLMODAL or MB_DEFBUTTON1) = idYes) then
      Exit;

    CM.AbreAviso('Calculando Média de Venda...');
    if not EstaTabela.IB_Transaction.InTransaction then EstaTabela.IB_Transaction.StartTransaction;
    EstaTabela.Active := False;
    EstaTabela.SQL.Clear;
    ...
    begin
      fDB.DefineValores(DtM.IBDatabaseServer,
                   ...
                   ['ID',                          0,
                    ...
                    'BLOQ_QUEM',                   0,
                    'MOSTRAMARGEMLUCRO',          'F',
                    'EXIBMARGEMLUCRO',            '#@@##@$;|',  // NÃO PODE COMEÇAR COM ARROBA!!!!
                    ...
                    'GRADECAMPOCODIGO',            'Código',
                    'GRADECAMPODESCRICAO',         'Descrição',
                    ...
                    'LANC_INVENTARIO',              2,
                    'PROD_DESC_COD02',             'Código 2',
                    'PROD_DESC_COD03',             'Código 3',
                    'PROD_DESC_COD04',             'Código 4'],   '', true);
    ...
    begin
      fDB.DefineValores(DtM.IBDatabaseServer,
                   ...
                   ['NUM_TERMINAL',              0,
                    ...
                    'ICB_OBS1',                  'Trocas até 08 dias com',
...
begin
  ...
  try
    ...
    repeat
      ...
      if JaExiste then AbreAviso('Gerando Código ('+SCodSeq+')');
    until not JaExiste;
    FechaAviso;
    ...
    if (QuantDigTotal > 0) and (Length(CodGerado) > QuantDigTotal) then
      MensagemDeAtencao('A quantidade de dígitos do código gerado é superior à quantidade de dígitos definida nas configurações.');
  finally
    fDB.FechaTT(EstaTabela);
    fDB.FreeQueryTransacao(EstaTabela, EstaTransacao);
  end;
...
begin
  ...
  try
    ...
    // Copia os valores das Configurações Locais (RAIZ)...
    ...
    // Copia os valores das Configurações Locais (pesquisas)...
    ...
    // Copia os valores das Configurações Locais (RELATORIOS)...
    ...
    // Copia os valores das Configurações Locais (CAIXA)...
    ...
    // Copia os valores das Configurações Locais (BALANÇA)...
    ...
    // Copia os valores das Configurações Locais (BOLETO)...
    ...
    // Copia os valores das Configurações Locais (CARNÊ)...
    ...
    // Copia os valores das Configurações Locais (INDPAG)...
    ...
    // Copia os valores das Configurações Locais (RECIBO)...
    ...
    // Copia os valores das Configurações Locais (CUPOM)...
    ...
    // Copia os valores das Configurações Locais (ECF)...
    ...
    // Copia os valores das Configurações Locais (NOTA FISCAL)...
    ...
    // Copia os valores das Configurações Locais (PDV)...
...
begin
  try
    ...
    if not DtM.IBDatabaseServer.Connected then
    begin
      if tcExibeMensagens then AbreAviso('Estabelecendo conexão com o SERVIDOR FOCUS');
    ...
    Screen.Cursor := crDefault;
    FechaAviso;
    if tcExibeMensagens then MensagemDeAtencao('Não há conexão com o SERVIDOR FOCUS');
  end;
end;
...
begin
  ...
  if ((rgRestaurar) and (MessageBox(0, 'Deseja restaurar as configurações de colunas do Grid?', 'Restaurar Grid?', MB_ICONQUESTION or MB_YESNO or MB_TOPMOST or MB_APPLMODAL or MB_DEFBUTTON1) = idYes)) then
  begin
...
begin
  try
    ...
    case CST[1] of
      ...
      '1': result := 'Origem Estrangeira (Importação Direta)';
    ...
    case StrToIntZ(CST[2]+CST[3], 0) of
      ...
      10 : result := result +PL+ 'Tributada e com cobrança do ICMS por substituição tributária';
      20 : result := result +PL+ 'Com redução de base de cálculo';
      30 : result := result +PL+ 'Isenta ou não tributada e com cobrança do ICMS por substituição tributária';
      40 : result := result +PL+ 'Isenta';
      41 : result := result +PL+ 'Não Tributada';
      50 : result := result +PL+ 'Suspensão';
      51 : result := result +PL+ 'Diferimento';
      60 : result := result +PL+ 'ICMS cobrado anteriormente por substituição tributária';
      70 : result := result +PL+ 'Com redução de base de cálculo e cobrança do ICMS por substituição tributária';
...
begin
  MensagemDeInformacao('CÓDIGO DE SITUAÇÃO TRIBUTÁRIA'+PL+
                          '--------------------------------------------------------------'+PL+
                          '0 = Origem Nacional'+PL+
                          '1 = Origem Estrangeira - Importação direta'+PL+
                          '2 = Origem Estrangeira - Adquirida no mercado interno'+PL+
                          '--------------------------------------------------------------'+PL+
                          '00 = Tributada integralmente'+PL+
                          '10 = Tributada e com cobrança do ICMS por substituição tributária'+PL+
                          '20 = Com redução de base de cálculo'+PL+
                          '30 = Isenta ou não tributada e com cobrança do ICMS por substituição tributária'+PL+
                          '40 = Isenta'+PL+
                          '41 = Não Tributada'+PL+
                          '50 = Suspensão'+PL+
                          '51 = Diferimento'+PL+
                          '60 = ICMS cobrado anteriormente por substituição tributária'+PL+
                          '70 = Com redução de base de cálculo e cobrança do ICMS por substituição tributária'+PL+
                          '90 = Outras' );
end;
