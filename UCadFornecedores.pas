...
type
  ...
  public
    RegDadosInclusao: TRegDadosInclusao;
    { Public declarations }
  end;
...
begin
  if (Modo in [mdInclusao, mdInclusaoXML, mdAlteracao]) and (Vazio(DBEdit3.Text)) then
...
begin
  ...
  PageControl1.ActivePage := TabSheet1;
  inherited;
  with EstaTabela, RegDadosInclusao do
  begin
    FieldByName('CGC').AsString := CGC;
    FieldByName('INSCESTADUAL').AsString := IE;
    FieldByName('NM_FORNECEDOR').AsString := xNome;
    FieldByName('NOMEFANTASIA').AsString := xFant;
//    FieldByName('CGC').AsString := IEST;
    FieldByName('INSC_MUNICIPAL').AsString := IM;
    FieldByName('CNAE_FISCAL').AsString := CNAE;
    FieldByName('CRT').AsString := CRT;
    FieldByName('TELEFONE').AsString := Fone;
    FieldByName('CEP').AsString := CEP;
    FieldByName('ENDERECO').AsString := xLgr;
    FieldByName('ENDERECO_NUMERO').AsString := nro;
    FieldByName('COMPLEMENTO').AsString := xCpl;
    FieldByName('BAIRRO').AsString := xBairro;
    FieldByName('CD_MUNICIPIO').AsString := cMun;
    FieldByName('CIDADE').AsString := xMun;
    FieldByName('ESTADO').AsString := UF;
    FieldByName('CD_PAIS').AsString := cPais;
  end;
  DBEdit2.SetFocus;
end;
...
begin
  ItemEsc := mPesq.Pesquisa(pqRepresentantes, [mdInclusao, mdInclusaoXML, mdAlteracao]);
...
var
  Codigo : Integer;
begin
  if (Modo = mdInclusao) or (Modo = mdInclusaoXML) then
  begin
...
begin
  if (Modo in [mdInclusao, mdInclusaoXML, mdAlteracao]) then
  begin
...
begin
  if (CM.F3OuAltDown(key, shift)) and (Modo in [mdAlteracao, mdInclusao, mdInclusaoXML]) then btPesquisaMunicipio.Click;
end;
...
begin
  ...
  if (Modo in [mdAlteracao, mdInclusao, mdInclusaoXML]) then
  begin
